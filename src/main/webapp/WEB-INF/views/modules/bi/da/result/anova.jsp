				...
				<div class="widget-main no-padding">
					<table class="table table-striped table-bordered table-hover">
						<tbody>
							...
							<tr>
								<td class=""><spring:message code="da.result.normal.choose.formula"></spring:message></td>
								<td>
								${dfi.chosenParams[fn:length(dfi.chosenParams)-1].column.modelColumnNameLab }${dfi.formulaSymbol[fn:length(dfi.formulaSymbol)-1] }
									<c:forEach items="${dfi.chosenParams }" var="cps" varStatus="stat" begin="0" end="${fn:length(dfi.chosenParams)-2 }">
										${cps.column.modelColumnNameLab } 
										<c:if test="${stat.index<(fn:length(dfi.formulaSymbol)-1) }">
										${dfi.formulaSymbol[stat.index] }
										</c:if>
									</c:forEach>
								</td>
							</tr>
							...
							<tr>
								<td>P.Value</td>
								<td>
									<c:forEach items="${rResult.resultMap.pValue}" var="item" varStatus="status">
										<div class="row">
											<div class="col-xs-3 classModelColumn">
											${dfi.chosenParams[status.index].column.modelColumnNameLab }
											</div>
											<div class="col-xs-1">
												=
											</div>
											<div class="col-xs-8">
												${fnbi:keepTwoDecimal(item)}&nbsp;&nbsp;${rResult.resultMap.SignificanceLevel[status.index]}
											</div>
										</div>
									</c:forEach>
								</td>
							...
							<tr>
								<td>Signif.codes</td>
								<td>
									${rResult.resultMap.SignificanceCodes}
								</td>
							</tr>
						</tbody>
					</table>
...
<!-- <script>
$(function(){
	$("#model_column_param_div").find("select[name='model_column_sels']").each(function(i,item){

		$($(".classModelColumn")[i]).html($(this).find("option:selected").text());
	});
