							...
							<tr>
								...
								<td>
									<table frame="below" border="0"> 
										<%-- <tr>
											<c:forEach items="${rResult.resultMap.predictResult }" var="item">
												<td>${dfi.chosenParams[fn:length(dfi.chosenParams)-1].column.modelColumnNameLab }</td>
											</c:forEach>
										</tr> --%>
										<tr>
											<td></td>
										...
										<c:forEach items="${rResult.resultMap.predictResult }" var="item" varStatus="itemSta">
											<tr>
											<td>${rResult.resultMap.rowNameArray[itemSta.index] }</td>
