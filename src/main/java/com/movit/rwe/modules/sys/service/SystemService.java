...
import com.movit.rwe.common.security.Digests;
import com.movit.rwe.common.security.shiro.session.SessionDAO;
import com.movit.rwe.common.service.BaseService;
import com.movit.rwe.common.service.ServiceException;
import com.movit.rwe.common.utils.CacheUtils;
import com.movit.rwe.common.utils.Encodes;
...
import com.movit.rwe.modules.sys.security.SystemAuthorizingRealm;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public Page<User> findUser(Page<User> page, User user) {
		...
		for(User tUser : userList) {
			tUser.setStudyList(userStudyService.queryListStudyByUserId(tUser.getId()));
			tUser.setRoleList(roleDao.getRoleListByUser(tUser.getId()));
		}
	...
	public void saveUser(User user) {
		...
		if (StringUtils.isNotBlank(user.getId())){
			...
			// 设置用户与study关联
			userStudyDao.deleteUserStudyByUser(user.getId());
			List<HiveStudy> studyList = user.getStudyList();
			if(studyList != null && studyList.size() > 0) {
				for(HiveStudy study : studyList) {
					UserStudy userStudy = new UserStudy();
					userStudy.setUserId(user.getId());
					userStudy.setStudyId(study.getGuid());
					userStudy.preInsert();
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void updateLocaleById(String id, String locale) {
		User user = new User(id);
		user.setLocale(locale);
		userDao.updateLocaleById(user);
		// 清除用户缓存
		UserUtils.clearCache(user);
	}
